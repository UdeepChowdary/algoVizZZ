      ...
      <div className="flex flex-wrap justify-center gap-4">
        {items.map((it, idx) => {
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between font-bold text-sm text-indigo-300 border-b border-slate-800 pb-1">
                <span className="flex items-center gap-1 font-sans">
                  <Package className="w-4 h-4 text-indigo-300" /> Item #{it.id}
                </span>
              ...
              <div>Value: <span className="text-emerald-300 font-bold">${it.val}</span></div>
      ...
      <div className="w-full max-w-[480px] bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col gap-2 font-sans">
        <div className="flex justify-between text-xs text-slate-300 font-bold">
          ...
          <span className="font-mono">
            <span className="text-emerald-300">{usedCap.toFixed(1)}kg</span> / {initialCap}kg
          </span>
        </div>
        ...
        <div className="flex justify-between text-[11px] text-slate-300 font-mono">
          <span>Remaining Capacity: <strong className="text-rose-300">{capacity.toFixed(1)} kg</strong></span>
          <span>Total Profit: <strong className="text-emerald-300">${step.totalVal?.toFixed(2) || 0}</strong></span>
        </div>
      </div>
    </div>
  );
